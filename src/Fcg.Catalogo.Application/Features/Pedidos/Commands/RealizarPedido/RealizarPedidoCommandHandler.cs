...
namespace Fcg.Catalogo.Application.Features.Pedidos.Commands.RealizarPedido
{
    public class RealizarPedidoCommandHandler : IRequestHandler<RealizarPedidoCommand, bool>
    {
        ...
        {
            _logger.LogInformation("[CatalogoAPI]  Iniciando processamento do pedido para o usuário: {UsuarioId}", request.UsuarioId);
            ...
            if (idsInexistentes.Any())
            {
                ...
                _logger.LogWarning("[CatalogoAPI]  Pedido negado. Os seguintes jogos não foram encontrados: {IdsInexistentes}", idsFormatados);
            ...
            foreach (var jogo in jogos)
            {
                ...
                {
                    _logger.LogWarning("[CatalogoAPI]  Pedido negado. Usuário {UsuarioId} já possui o jogo {JogoId}", request.UsuarioId, jogo.Id);
                    throw new DomainException($"O usuário já possui o jogo {jogo.Nome.Valor} em sua biblioteca.");
                }

                if (!jogo.Ativo)
                {
                    _logger.LogWarning("[CatalogoAPI]  Pedido negado. Jogo {JogoId} inativo", jogo.Id);
            ...
            _logger.LogInformation("[CatalogoAPI]  Validações concluídas. Publicando OrderPlacedEvent (OrderId: {OrderId}, Total: {PrecoTotal})", orderId, precoTotal);
