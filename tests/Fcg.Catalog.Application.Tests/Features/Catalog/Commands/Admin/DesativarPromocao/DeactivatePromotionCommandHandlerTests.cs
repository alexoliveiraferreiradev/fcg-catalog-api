using Fcg.Catalog.Application.Features.Catalog.Commands.Admin.DeactivatePromotion;
using Fcg.Catalog.Domain.Entities;
using Fcg.Catalog.Domain.Enum;
using Fcg.Catalog.Domain.Events;
using Fcg.Catalog.Domain.Repositories;
using Fcg.Catalog.Domain.ValueObject;
using Fcg.Core.Abstractions.Common.Exceptions;
using Fcg.Core.Abstractions.Interfaces;
using FluentAssertions;
using MediatR;
using Microsoft.Extensions.Logging;
using Moq;
using System;
using System.Linq;
using System.Threading;
using System.Threading.Tasks;
using Xunit;

namespace Fcg.Catalog.Application.Tests.Features.Catalog.Commands.Admin.DeactivatePromotion
{
    public class DeactivatePromotionCommandHandlerTests
    {
        private readonly Mock<IGameRepository> _jogoRepositoryMock;
        private readonly Mock<IUnitOfWork> _unitOfWorkMock;
        private readonly Mock<ILogger<DeactivatePromotionCommandHandler>> _loggerMock;
        private readonly Mock<IMediator> _mediatorMock;
        private readonly DeactivatePromotionCommandHandler _handler;

        public DeactivatePromotionCommandHandlerTests()
        {
            _jogoRepositoryMock = new Mock<IGameRepository>();
            _unitOfWorkMock = new Mock<IUnitOfWork>();
            _loggerMock = new Mock<ILogger<DeactivatePromotionCommandHandler>>();
            _mediatorMock = new Mock<IMediator>();

            _handler = new DeactivatePromotionCommandHandler(
                _jogoRepositoryMock.Object,
                _unitOfWorkMock.Object,
                _loggerMock.Object,
                _mediatorMock.Object
            );
        }

        private Game CriarJogoValido()
        {
            return new Game(
                new Name("Game Teste"),
                new Description("Description do Game longa"),
                new Price(100.00m),
                GameGenre.RPG
            );
        }

        [Fact]
        public async Task Handle_DeveDeactivatePromotion_QuandoExistirParaOJogo()
        {
            // Arrange
            var Game = CriarJogoValido();
            Game.AddPromotion(new Price(50.0m), new Period(DateTime.UtcNow.AddDays(10)));
            var Promotion = Game.Promotions.First();

            var command = new DeactivatePromotionCommand { PromotionId = Promotion.Id };

            _jogoRepositoryMock.Setup(r => r.GetPromotionById(command.PromotionId)).ReturnsAsync(Promotion);
            _jogoRepositoryMock.Setup(r => r.GetById(Game.Id)).ReturnsAsync(Game);

            // Act
            await _handler.Handle(command, CancellationToken.None);

            // Assert
            Promotion.IsActive.Should().BeFalse();
            _jogoRepositoryMock.Verify(r => r.Update(Game), Times.Once);
            _unitOfWorkMock.Verify(u => u.CommitAsync(), Times.Once);
            _mediatorMock.Verify(m => m.Publish(It.IsAny<PromotionDeactivatedEvent>(), It.IsAny<CancellationToken>()), Times.Once);
        }

        [Fact]
        public async Task Handle_DeveLancarDomainException_QuandoPromocaoNaoForEncontrada()
        {
            // Arrange
            var command = new DeactivatePromotionCommand { PromotionId = Guid.NewGuid() };
            _jogoRepositoryMock.Setup(r => r.GetPromotionById(command.PromotionId)).ReturnsAsync((Promotion)null);

            // Act
            Func<Task> act = async () => await _handler.Handle(command, CancellationToken.None);

            // Assert
            await act.Should().ThrowAsync<DomainException>();
        }
    }
}
