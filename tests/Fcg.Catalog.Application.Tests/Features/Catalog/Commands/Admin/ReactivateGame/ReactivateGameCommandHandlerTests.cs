...
{
    public class ReactivateGameCommandHandlerTests
    {
        ...
        [Fact]
        public async Task Handle_DeveReactivateGameEPublicarEvento_QuandoJogoExistirEEstiverInativo()
        {
            // Arrange
            var Game = CriarJogoValido();
            Game.Deactivate(); // Deixa inativo primeiro
            var command = new ReactivateGameCommand { GameId = Game.Id };
        ...
        {
            // Arrange
            var command = new ReactivateGameCommand { GameId = Guid.NewGuid() };
